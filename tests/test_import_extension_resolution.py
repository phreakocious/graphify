...
Adapted from upstream PR #717 (merged into upstream/v7) to navigator (v4-
based, no `extract_svelte` and no `_dynamic_import_js` handler). The
resolver itself + `_import_js` static-import path apply unchanged; the
3 dynamic_import tests in the original PR are dropped because the v4
extractor doesn't have a separate dynamic-import handler to exercise.
"""
