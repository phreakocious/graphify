...
def load_graph(graph_path: str | Path,
    ...
    # cli_auto_build candidate: when graph.json is missing, auto-run
    # `graphify update <root>` once before reading. Fixes the bimodal
    # failure mode the meta-harness measured on abl_no_first_call_hygiene
    # — agents who didn't realize they needed `graphify update` burned
    # 3× the tokens floundering through read_file. Build location is
    # the parent of `graphify-out/` (the repo root by convention).
    if not path.exists():
        import subprocess as _subprocess
        repo_root = path.parent.parent
        if not repo_root.exists() or not repo_root.is_dir():
            repo_root = Path.cwd()
        print(f"⏵ graph not found at {path}; running `graphify update {repo_root}` "
              "to auto-build…", file=sys.stderr)
        try:
            proc = _subprocess.run(
                ["graphify", "update", str(repo_root)],
                capture_output=True,
                text=True,
                timeout=180,
            )
            ...
            raise FileNotFoundError(
                f"auto-build failed ({type(exc).__name__}: {exc}); "
                f"run `graphify update {repo_root}` manually."
            ) from exc
        if proc.returncode != 0 or not path.exists():
            err_tail = (proc.stderr or proc.stdout or "")[-500:]
            raise FileNotFoundError(
                f"auto-build returned {proc.returncode} but graph still not found "
                f"at {path}. last stderr/stdout: {err_tail}"
            )
