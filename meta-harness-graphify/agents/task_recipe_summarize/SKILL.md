...
| **Summarize a class** (signature + methods + used-by + inheritance) | `graphify summarize "@<Class>"` — fuses class signature, full method list, cross-file callers, and parent/sibling/child classes into one call. Use this when you need class-level context — do **not** fall back to `read_file` on the file holding the class. |
