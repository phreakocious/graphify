...
| `graphify blast "<symbol>"` | one-shot blast radius — callers + callees side-by-side under `## Callers` / `## Callees` markdown sections. Cursor-free. Best for refactor planning ("what calls X and what does X call?"). Accepts the same path-qualified disambig as `peek`/`navigate`. |
