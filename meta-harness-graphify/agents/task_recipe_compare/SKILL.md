...
**Before each call, name two ways you could answer the current sub-question and pick the cheaper one.** Often the alternatives are "graphify recipe X" vs "read_file directly" or "graphify recipe X" vs "graphify recipe Y". Naming them prevents iterating through the wrong one. Don't run both — pick.
