...
**Session priming — read this first.** When you start work on a new file or symbol, your FIRST call should be `graphify shape <file>` (for files), `graphify summarize @<Class>` (for classes), or `graphify navigate "@<symbol>"` (for free symbols). This puts structural data — line ranges, method signatures, call structure, entry points — into the conversation cache where every follow-up question can reuse it for free. Reading file bytes early fills cache with raw text that doesn't compose across questions. The first graphify call typically pays for itself within the next 2-3 questions in the same session.
